...
const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ documents, onClose }) => {
  ...
  useEffect(() => {
    ...
    const nodes: GraphNode[] = documents.map((doc, index) => {
      ...
      const radius = Math.min(300, 200 + documents.length * 5);
    ...
    for (let i = 0; i < documents.length; i++) {
      for (let j = i + 1; j < documents.length; j++) {
        ...
        // 相关文档引用增加连接强度
        if (doc1.relatedDocuments.some(rel => doc2.title.includes(rel))) {
          strength += 0.4;
        }
        if (doc2.relatedDocuments.some(rel => doc1.title.includes(rel))) {
          strength += 0.4;
        }
        
        // 内容关键词相似性
        const keywords1 = extractKeywords(doc1.content);
        const keywords2 = extractKeywords(doc2.content);
