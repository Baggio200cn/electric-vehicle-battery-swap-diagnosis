...
class PDFDownloader:
    ...
    def download_pdf(self, url, title, progress_callback=None):
        """下载PDF文件"""
        try:
            # 清理文件名
...
class DownloadWorker(QThread):
    ...
    def run(self):
        """运行下载"""
        try:
            downloader = PDFDownloader(self.download_path)
            total_papers = len(self.papers)
            downloaded_count = 0
            
            for i, paper in enumerate(self.papers):
                if paper['pdf_url']:
                    self.status.emit(f"正在下载: {paper['title'][:50]}...")
                    
                    def progress_callback(progress):
                        self.progress.emit(int((i / total_papers) * 100 + (progress / total_papers)))
                    ...
                    if filepath:
                        downloaded_count += 1
                        self.status.emit(f"已下载: {paper['title'][:50]}...")
                    else:
                        self.status.emit(f"下载失败: {paper['title'][:50]}...")
                else:
                    self.status.emit(f"无PDF链接: {paper['title'][:50]}...")
                
                # 更新总进度
                self.progress.emit(int(((i + 1) / total_papers) * 100))
                
            self.status.emit(f"下载完成！成功下载 {downloaded_count}/{total_papers} 篇论文")
            
        except Exception as e:
            self.status.emit(f"下载出错: {str(e)}")
        finally:
            self.finished.emit()
...
class MachineVisionLiteratureApp(QMainWindow):
    ...
    def init_ui(self):
        """初始化用户界面"""
        self.setWindowTitle("机器视觉文献获取系统 - 增强版")
        ...
        main_layout.addLayout(download_panel)
    ...
    def create_control_panel(self):
        ...
        self.count_combo.addItems(["5", "10", "20", "50"])
    ...
    def create_download_panel(self):
        ...
        # 自动下载选项
        self.auto_download_check = QCheckBox("搜索完成后自动下载PDF")
        self.auto_download_check.setChecked(True)
        layout.addWidget(self.auto_download_check)
        
        # 下载路径选择
        path_layout = QHBoxLayout()
        self.download_path_label = QLabel("下载路径: ./downloads")
        ...
        # 手动下载按钮
        self.download_button = QPushButton("下载当前结果PDF")
        self.download_button.clicked.connect(self.start_download)
        self.download_button.setEnabled(False)
        self.download_button.setStyleSheet("QPushButton { background-color: #2196F3; color: white; font-size: 14px; padding: 10px; }")
    ...
    def select_download_path(self):
        ...
        if path:
            self.download_path_label.setText(f"下载路径: {path}")
    ...
    def start_download(self):
        ...
        if not self.current_papers:
            QMessageBox.warning(self, "提示", "没有可下载的论文")
            return
            
        download_path = self.download_path_label.text().replace("下载路径: ", "")
    ...
    def display_results(self, papers):
        ...
        for i, paper in enumerate(papers, 1):
            ...
            if paper['pdf_url']:
                ...
                self.result_text.append("PDF链接: 无")
    ...
    def search_finished(self):
        ...
        if self.current_papers:
            ...
            if self.auto_download_check.isChecked():
                self.update_status("开始自动下载PDF...")
                QTimer.singleShot(1000, self.start_download)  # 1秒后开始下载
    
    def download_finished(self):
        """下载完成"""
        self.download_button.setEnabled(True)
        self.progress_bar.setValue(100)
        self.update_status("PDF下载完成")
        
        # 显示下载完成消息
        download_path = self.download_path_label.text().replace("下载路径: ", "")
        QMessageBox.information(self, "下载完成", f"PDF文件已下载到:\n{download_path}")
