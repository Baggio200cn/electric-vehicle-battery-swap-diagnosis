...
function App() {
  ...
  const generateLogDescription = (logData: {
    ...
    if (logData.result.recommendations && logData.result.recommendations.length > 0) {
      description += `\n解决方案: ${logData.result.recommendations.join(', ')}`;
    }
    
    if (logData.result.summary) {
      description += `\n分析描述: ${logData.result.summary}`;
    }
  ...
  const handleTextAnalysis = async (text: string) => {
    ...
    try {
      ...
      // 将TextAnalysisResponse转换为DiagnosisResult格式
      const diagnosisResult: DiagnosisResultType = {
        type: 'text',
        confidence: response.analysis.confidence,
        issues: [response.analysis.faultType],
        recommendations: response.analysis.solutions,
        summary: response.analysis.solutions.join('; '),
        severity: response.analysis.confidence > 0.8 ? 'high' : response.analysis.confidence > 0.5 ? 'medium' : 'low',
        timestamp: new Date(),
  ...
  const handleVideoAnalysis = async (file: File) => {
    ...
    try {
      ...
      // 将VideoAnalysisResponse转换为DiagnosisResult格式
      const diagnosisResult: DiagnosisResultType = {
        type: 'video',
        confidence: response.analysis.confidence,
        issues: [response.analysis.faultType],
  ...
  const handleAudioAnalysis = async (file: File) => {
    ...
    try {
      ...
      const result: DiagnosisResultType = {
        ...
        summary: '检测到音频信号异常，可能存在设备故障',
        severity: 'medium',
        timestamp: new Date(),
        fileName: file.name,
        analysisType: 'audio-analysis'
      };
      ...
      const audioStats: Statistics = {
        totalTests: 1,
        passedTests: result.confidence > 0.7 ? 0 : 1,
        failedTests: result.confidence > 0.7 ? 1 : 0,
        warningTests: 0,
        totalFrames: 1,
        analyzedFrames: 1,
  ...
  const handleImageUpload = async (files: File[], analysisData?: any) => {
    ...
    try {
      ...
      const imageStats: Statistics = {
        ...
        passedTests: files.length - abnormalCount,
        failedTests: abnormalCount,
        warningTests: 0,
  ...
  const convertImageAnalysisToResult = (analysisData: any, files: File[]): DiagnosisResultType => {
    ...
    // 生成解决方案
    const recommendations = prioritizedSolutions?.map((sol: any) => sol.description) || [
      '继续监控设备状态',
      '按计划进行维护保养',
      '记录设备运行参数'
    ];
    ...
    return {
      type: files.length > 1 ? 'image' : 'image',
      confidence,
      issues: [faultType],
      ...
      analysisType: files.length > 1 ? 'multi-image' : 'single-image'
    };
  };
  ...
  const performAdvancedImageAnalysis = async (files: File[]): Promise<DiagnosisResultType> => {
    ...
    const recommendations = generateRecommendations(allIssues, matchedKnowledge);
    ...
    return {
      ...
      issues: [faultType],
      recommendations,
      summary: generateSummary(files.length, allIssues, matchedKnowledge),
      severity,
      timestamp: new Date(),
      fileName: files[0]?.name,
      analysisType: files.length > 1 ? 'multi-image' : 'single-image'
  ...
  // 生成解决方案
  const generateRecommendations = (issues: string[], knowledge: KnowledgeDocument[]): string[] => {
    const recommendations = [];
    
    if (issues.includes('腐蚀')) {
      recommendations.push('立即清除腐蚀物质，重新涂抹防腐涂层');
      recommendations.push('检查环境湿度控制系统');
      recommendations.push('加强防水密封措施');
    }
    
    if (issues.includes('裂纹')) {
      recommendations.push('停机检查，评估裂纹扩展风险');
      recommendations.push('进行无损检测，确定裂纹深度');
      recommendations.push('必要时更换受损部件');
    }
    
    if (issues.includes('磨损')) {
      recommendations.push('检查润滑系统，补充润滑油');
      recommendations.push('调整设备运行参数，减少磨损');
      recommendations.push('制定部件更换计划');
    }
    
    if (issues.includes('过热')) {
      recommendations.push('检查冷却系统运行状态');
      recommendations.push('清理散热器，确保通风良好');
      recommendations.push('监控负载，避免过载运行');
    }
    ...
    return recommendations.length > 0 ? recommendations.slice(0, 5) : ['设备状态良好，继续正常运行'];
  };
  ...
  // 生成描述
  const generateSummary = (fileCount: number, issues: string[], knowledge: KnowledgeDocument[]): string => {
